...
- Rust extension helper: `cmake/bsk_add_rust_module.cmake`
...
The default command initializes `external/basilisk` when needed and restores
the exact commit recorded by the SDK repository. An explicitly selected
checkout outside that submodule is never moved. A standalone Git clone placed
at `external/basilisk` by CI is also preserved.
...
For an intentional update of the recorded Basilisk submodule commit:

```bash
git -C external/basilisk fetch origin
git -C external/basilisk checkout <exact-commit-id>
git add external/basilisk
python3 tools/sync_all.py
```

Use `git -C <basilisk-checkout> rev-parse HEAD` to obtain the exact commit ID.
`HEAD` refers to the currently checked-out commit in the repository named by
`-C`; it does not include uncommitted edits. Use `--no-sync-submodules` only
when deliberately testing an unrecorded commit inside `external/basilisk`.
...
`src/bsk_sdk/rust/support-manifest.txt` is generated by `sync_rust.py` and is
the authoritative list of Rust support files required by the build backend and
...
pytest examples/custom-atm-extension/rustAtmosphere/_UnitTest/test_rustAtmosphere.py -v
```
...
- The example extension includes a Rust module and therefore needs Rust 1.89 or
  newer. Extensions that do not call `bsk_add_rust_workspace` do not need Rust.
...
- Reject every Basilisk/BSK-SDK version mismatch, including `alpha`, `beta`,
  and `rc` versions. Pre-release builds are more likely to contain incompatible
  native changes, so a warning is not sufficient protection. For local branch
  testing, synchronize BSK-SDK from the same Basilisk checkout used to build
  and install Basilisk rather than weakening the version guard.
